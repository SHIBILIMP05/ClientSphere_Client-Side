...
const EmployeeCard: React.FC<EmployeeCardProps> = ({ name, phone, email, imageUrl, id,is_restricted, is_block, setIs_block }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-4 flex flex-col items-center text-center ">
      {imageUrl ? <img src={imageUrl} alt={name} className="w-24 rounded-full mb-4 object-cover aspect-square" /> : <img src={`https://ui-avatars.com/api/?name=${name}&background=random`} alt={name} className="w-24 h-24 rounded-full mb-4 object-cover aspect-square" />}
